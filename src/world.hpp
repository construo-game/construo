...
public:
  ...
  Particle* find_particle(geom::fpoint const& pos, float capture_distance = 20.0f) const;
  /** All particles inside the given rectangle */
  std::vector<Particle*> find_particles(float x1, float y1, float x2, float y2) const;
  Spring* find_spring(geom::fpoint const& pos, float capture_threshold = 20.0f) const;
