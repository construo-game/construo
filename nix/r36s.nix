...
#   nix build .#construo-r36s
#   nix build .#construo-r36s-portmaster   # PortMaster tree for /roms/ports
#
{ lib
, stdenv
, stdenvNoCC
...
let
  ...
  #   - -fexceptions: Construo/tinygettext/prio use try/catch (unlike SuperTux M1)
  ...
  mkWrappers = sysroot: let
    ...
    # -fexceptions: tinygettext / prio / Construo use C++ exceptions; pair with
  ...
  , pname ? "construo-r36s"
    ...
    stdenv.mkDerivation {
      ...
      cmakeFlags = [
        ...
        "-DWARNINGS=OFF"
        "-DWERROR=OFF"
        # R36S: SDL2 + GLES2 only (no X11/GLUT).
      ...
      preConfigure = ''
        ...
        if false; then  # construo: no OpenAL/modplug
          OPENAL_LIB=
          OPENAL_INC=
          for cand in \
      ...
      postInstall = ''
        mkdir -p $out/share/construo $out/lib/construo
        # CMake may have installed data/ as non-writable; ensure we can add files.
        chmod -R u+w $out/share/construo || true
        if [ -d "$src/examples" ]; then
          mkdir -p $out/share/construo/examples
          cp -a "$src/examples/." $out/share/construo/examples/ || true
          chmod -R u+w $out/share/construo || true
        fi
        ...
        if false; then  # construo: no OpenAL/modplug
          ...
          chmod -R u+w "$out/lib/construo" || true
          ls -la "$out/lib/construo" || true
        fi
        cat > $out/share/construo/README-R36S.txt << EOF_README
Construo — R36S / ArkOS (sysroot-linked)
====================================================

Binary: bin/construo.sdl (SDL2 + GLES2)
  Linked against the ArkOS aarch64 sysroot (SDL2 + OpenGL/GLES as available).

Deploy the binary + share/construo data to the device.

Controls (important)
--------------------
Construo uses the SDL Joystick API (raw indices). PortMaster still exports
SDL_GAMECONTROLLERCONFIG for other ports and for a future GameController path.

  Preferred: nix build .#construo-r36s-portmaster
...
EOF_README
        cat > $out/share/construo/construo.sh << 'LAUNCH'
#!/bin/bash
...
LAUNCH
      ...
      meta = with lib; {
        description = "Construo for R36S/ArkOS (sysroot-linked aarch64)";
  ...
  # Wrap a mkConstruoR36s result as a PortMaster-ready tree:
  #   Construo.sh + construo/ + metadata
  # Copy into /roms/ports/ or zip for PortMaster autoinstall.
  mkConstruoR36sPortMaster = {
    r36sPkg
  , version
  , pname ? "construo-r36s-portmaster"
  , title ? "Construo"
  , scriptName ? "Construo.sh"
  , portDirName ? "construo"
  , screenshotSrc ? ../data/icons/hicolor/128x128/apps/construo.png
  }:
    stdenvNoCC.mkDerivation {
      ...
      installPhase = ''
        ...
        if [ -d "${r36sPkg}/lib/construo" ]; then
          mkdir -p "$gamedir/libs"
          cp -a "${r36sPkg}/lib/construo/." "$gamedir/libs/"
          chmod -R u+w "$gamedir/libs" || true
        fi

        # Game data (CMake DATA_PREFIX was share/construo).
        # Store paths are mode 444/555; make writable so we can drop helpers.
        if [ -d "${r36sPkg}/share/construo" ]; then
          cp -a "${r36sPkg}/share/construo/." "$gamedir/data/"
          chmod -R u+w "$gamedir/data"
          # Keep only game assets under data/; drop packaging helpers
          rm -f "$gamedir/data/construo.sh" \
...
Construo — see upstream GPL-3.0-or-later and LICENSES/ in the source tree.
...
#!/bin/bash
# Construo — PortMaster launcher for R36S / ArkOS
...
GAMEDIR="/$directory/ports/construo"
...
1. Copy `Construo.sh` and the `construo/` directory to `/roms/ports/` on the device, **or**
2. Zip this folder and place the zip in `ports/PortMaster/autoinstall/`, then open PortMaster once.

### Controls (required)

Construo reads the pad via the **SDL Joystick API** (raw button/axis indices).
`SDL_GAMECONTROLLERCONFIG` is still exported by this launcher for consistency
with other PortMaster ports; it does not remap Construo’s joystick bindings.
...
Thanks to the Construo developers and the PortMaster / ArkOS communities.
EOF_README

        '';

      meta = with lib; {
        description = "PortMaster package of Construo for R36S/ArkOS";
        license = licenses.gpl3Plus;
        platforms = platforms.linux;
        hydraPlatforms = [];
      };
  ...
  mkConstruoR36sPortMasterZip = {
    portMasterPkg
  , version
  , pname ? "construo-r36s-portmaster-zip"
  , zipName ? "construo.zip"
  }:
    stdenvNoCC.mkDerivation {
      inherit pname version;
      dontUnpack = true;
      ...
      meta = with lib; {
        description = "PortMaster autoinstall zip of Construo for R36S/ArkOS";
        license = licenses.gpl3Plus;
        platforms = platforms.linux;
        hydraPlatforms = [];
      };
...
in
{
  inherit arkosSysroot mkConstruoR36s mkConstruoR36sPortMaster mkConstruoR36sPortMasterZip;
}
